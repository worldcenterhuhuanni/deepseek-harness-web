...
| `retryOnBlockedPage` | `true` | 站点拒绝发送时按重试策略重发本轮，并先换一条网页对话 |
| `sendableTimeoutMs` | `5000` | 等发送键变为可用的上限；超时即按「站点禁用了发送」处理 |
| `submitTimeoutMs` | `3000` | 等一种提交方式生效的上限，超时换下一种 |
| `attachTimeoutMs` | `120000` | 等附件上传并被站点解析完的上限；文件大或网络慢时调高 |
| `loginHints` | 见 `DEFAULT_PAGE_HINTS` | 判定未登录的页面提示词表 |
| `failHints` | 见 `DEFAULT_PAGE_HINTS` | 判定本次附件挂载失败的词表 |
| `busyHints` | 见 `DEFAULT_PAGE_HINTS` | 判定附件仍在上传或解析的词表 |
| `blockedHints` | 见 `DEFAULT_PAGE_HINTS` | 判定站点拒绝发送的词表；站点改了文案就在这里补 |
...
**失败记账有两个相互独立的维度：内容基线还能不能用，承载它的页面还能不能用。**

内容维度看提示词有没有已经进入对话（`submitted` 里程碑）。提交之前失败（登录过期、附件没挂上、找不到输入框）时页面没见过这一轮，基线仍然有效；提交之后失败则页面收到了多少、产出了什么都无从确认，基线必须作废，否则下一轮的增量建立在错的起点上。

通道维度看站点还愿不愿意让这个页面发消息。站点在输入区留下一张上传失败的附件卡片后会禁用发送键并要求先删掉它——此时对话内容一个字没变，基线完全有效，但这个页面再也发不出任何东西。只有内容维度时就会一直复用它：三种提交方式各试一遍全部失败，报一句「页面可能已改版」，重试再挂一次附件，于是残留越堆越多，从那一轮起每一轮都发不出去。所以 `page-blocked` 单独记在通道维度上，下一轮据此换一条网页对话（`newChat` + 全量），而不是继续用这个页面。
...
**「这一次没挂上」与「这个页面发不出去」是两种状态，判据也不同。** `failed` 命中的是本次挂载失败，重挂一次可能就好；`blocked` 命中的是输入区里还留着上一次的残留，站点会一直禁用发送键。前者按原错误分类重试，后者归 `page-blocked` 并触发换通道。

**词表全部来自配置，不写死在注入脚本里。** 站点改一次文案就够让写死的词表失灵——而这次的失败正是这样发生的：`请删除异常文件再发送` 不在原词表里，于是挂载被判为「已就绪」，直到三种提交方式全部失败才报出一句与原因无关的「页面可能已改版」。默认词表只有 `DEFAULT_PAGE_HINTS` 一处 home，Config 的四个 `*Hints` 字段引用它，用户在 cordis.yml 里补一条即刻生效，不必等插件发版。

**发不出去时必须能自证。** 判定与消息都取自当下的页面文本：命中拒绝词表就报站点原话并归 `page-blocked`，没命中就把输入区原文（`hint`）原样引进错误消息里。等发送键可用的那一步同样如此——等不到就说明站点主动禁用了它，这时三种提交方式都注定无效，报出页面的理由比试一遍再猜有用。
...
**协议必须写进输入框，而且注意力仍不归我们。** 站点自己的系统提示词压过我们说的任何话：把格式约定只放在附件里时，模型会从附件读到工具目录（它能叫出只可能来自那里的工具名），却把格式规则当成可以转述的说明文字——实测输出过 `[调用 glob] {"pattern": …}` 这种自创写法。所以请求按**内容角色**切分，而不是按传输方式：`preamble`（本轮提问 + 任务要求 + `TOOL_CALL_PROTOCOL`）恒定进输入框，`history`（对话正文与工具目录）才允许改走附件。即便如此也只是提高命中率，不是保证。历史里的调用**不用**这个格式回放，理由见下条。

**历史里的调用不按协议格式回放。** 用协议格式回放曾经是刻意的：让模型看见正确写法。但实测它会整段照抄——包括我们自己生成的 `id`（`web-<uuid12>-<name>`），而那种复述一旦被认出来，就会把同一处编辑重做第二遍。协议现在每轮都在 `preamble` 里，格式示范不再依赖历史，所以历史改成 `【已执行 <id> · <工具名>】` 加一行参数：可读、能与工具结果对上，又不像一条待发出的指令。
...
**因此解析器必须极度宽容。** 这不是防御式编程，是这条通路的固有条件——我们无法约束输出格式，只能识别模型实际使用的形式。目前认七种：
...
开场轮是系统指令、工具目录（每个工具的名称、描述与参数 JSON Schema）与全部对话记录，按 `### [n] 用户/助手/系统` 分节；续轮只有自上次发送以来新增的非 assistant 消息（新的 user 提问与工具结果）。历史里的工具调用渲染成 `【已执行 <id> · <工具名>】` 加一行参数，**不用**协议格式：用协议格式回放曾经是为了示范正确写法，但实测模型会整段照抄（连我们生成的 id 一起），那些复述一旦被认出来就会把同一处编辑重做一遍；格式示范已由每轮在场的 preamble 承担。图片块渲染成占位符，本通路不支持图片输入。
...
- **换了说法的拒绝提示只能靠词表认。** `blockedHints` 收的是生产环境见过的说法；站点用别的话拒绝时不会被判成 `page-blocked`，这一轮按 `TRANSPORT` 失败而不会换通道。错误消息仍然原样引用输入区文本，缺哪一句因此看得见——补进 `blockedHints` 是改配置，不是改代码。

...
**A failed turn does not necessarily cost the web conversation.** The test is whether the prompt already entered the conversation (the `submitted` milestone), not what kind of error occurred — the two have no causal relation. Failing before submission (an expired login, an attachment that never mounted, a composer that could not be found) leaves the page unaware of this turn and the conversation reusable; failing after submission leaves what the page received and produced unknowable, so the conversation is abandoned — otherwise the next increment builds on a wrong baseline.
